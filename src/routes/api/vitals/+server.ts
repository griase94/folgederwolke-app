...
export const POST: RequestHandler = async ({ request }) => {
  // In prod there is no consumer for these beacons, so we do NO work at all —
  // we don't even parse the body — and return 204 immediately. The parse +
  // allowlist + log only run in dev, where the structured line is read off
  // stdout. The endpoint contract (accept the beacon, 204) is identical in both.
  if (dev) {
    try {
