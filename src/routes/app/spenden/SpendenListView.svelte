<script lang="ts">
	/**
	 * SpendenListView — the flat Spenden list body, extracted verbatim from the
	 * list `+page.svelte` so it renders identically in two places:
	 *   - /app/spenden       — the real list page.
	 *   - /app/spenden/neu   — the Kulisse: an inert, aria-hidden backdrop behind
	 *                          the entry dialog.
	...
	let { data }: { data: SpendenListData } = $props();
			...
			>
				{#snippet pageActions()}
					...
					<Button
						...
						size="cta"
						data-slot="new-cta">Neue Spende</Button
					>
