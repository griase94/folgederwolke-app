<script lang="ts">
	 ...
	 *
	 * Renders the income Bezeichnung plus a read-only 🔗 "aus Rechnung" badge
	 * when (and only when) the row was created by the shipped `markInvoiceAsPaid`
	 * flow — i.e. `rechnungBusinessId` is non-null (the Phase-2 projection field).
	 * The badge is purely informational here (the detail page links to the
	 * Rechnung); it carries an accessible name "aus Rechnung {id}".
	 *
