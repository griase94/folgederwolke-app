<!--
  ...
  Same Buchung/Zuordnung card skeleton as Ausgabe/Einnahme (M1). Pickers
  (Spendenart / Zweckbindung / Spender) are neutral segmented toggles (never
  brand pink on a booking control — ANDY-LENS §4).
-->
<script lang="ts">
	...
	// from a re-hydrate in de-DE (comma) so it re-seeds exactly what the user sees;
	// track the parsed cents locally for the gate readout.
	const betragSeed = v('betragCents')
		? (Number(v('betragCents')) / 100).toFixed(2).replace('.', ',')
		: '';
	...
	// Betrag/Wert error (m7): 0,00 is client-submittable (gate = present) but flagged.
	const betragError = $derived(
		err('betragCents') ??
			(betragCents !== null && betragCents <= 0
				? `${isSach ? 'Gemeiner Wert' : 'Betrag'} muss größer als 0 sein.`
				: null),
	);
...
<div class="flex flex-col gap-4">
	<!-- ── Section 1: Buchung ─────────────────────────────────────────────────── -->
	<section class="rounded-xl border border-hairline bg-card/60 p-4" data-slot="spende-section">
		...
		<div class="flex flex-col gap-4">
			...
			<fieldset>
				...
				<!-- m9: the disabled reason is touch-visible, not only a title tooltip. -->
				<p class="mt-1.5 text-xs text-ink-500">
					Aufwandsspenden (Verzicht auf Erstattung) folgen in einer späteren Ausbaustufe.
				</p>
			...
			<!-- Sachspende Wertermittlung reveal / Geldspende optional Beleg -->
			{#if isSach}
				<fieldset
			...
			<div class="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 p-3">
				...
				<div class="min-w-0">
					...
					<p class="mt-0.5 text-xs leading-relaxed text-ink-500">
						...
						Wertermittlung (Beschreibung + gemeiner Wert).
					</p>
				</div>
			</div>
		</div>
	...
	<section class="rounded-xl border border-hairline bg-card/60 p-4" data-slot="spende-section">
		...
		<div class="flex flex-col gap-4">
			...
			<fieldset>
				<legend class="mb-1.5 block text-sm font-medium text-ink-900">
					Spender:in <span class="text-severity-critical" aria-hidden="true">*</span>
				</legend>
				<div class="{SEG_TRACK} mb-2" role="radiogroup" aria-label="Spender:in">
