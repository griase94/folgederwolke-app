...
> - **Double-audit RESOLVED (not a decision — a removal):** after delegating to `createDonation`, **REMOVE the `spende.created` emit** from `createSpende`. `donation.created` (fired inside `createDonation`) is the single append-only audit event (ADR-0004 — can't clean up later). No parallel `spende.created` event.
...
- [ ] **Step 0: Dependency gate (STOP if absent).** Task 4/5 depend on the Phase-1-amended `createDonation` (derives kategorie+`sphere='ideeller'`, accepts `wertermittlungMethode`/`zustandBeschreibung`/`herkunftsbelegFileId`/`belegFileId`) and `deriveDonationKategorieName`. Self-enforce: `grep -rn "deriveDonationKategorieName" src/lib/domain/spenden-kategorie.ts` and grep `createDonation` in `src/lib/server/domain/transactions.ts` for the derived/`wertermittlungMethode` signature, then run `pnpm check`. If either symbol is absent (or `pnpm check` fails on a missing signature) → **STOP** and escalate to Phase 1; do NOT begin the reconciliation.
...
- [ ] **Step 3: Implement.** In `spenden.ts`:
  - Extend `spendeInputSchema`: replace the legacy `sache_wertermittlung` enum + `sache_beschreibung` with the §4.3 fields — `wertermittlung_methode: z.enum(["marktpreis","kaufbeleg","schaetzung","buchwert"])` (required when `spende_kind==='sachspende'`), `zustand_beschreibung: z.string().min(3)` (required when sachspende), optional `herkunftsbeleg_file_id: z.string().uuid()`, optional main `beleg_file_id: z.string().uuid()`, optional `betriebsvermoegen: z.boolean().optional()` (SPEC-02 — only meaningful for Sachspende; defaults to `false`/Privatvermögen at the DB). **Remove** `kategorie_id` from the schema (no longer UI-supplied). Keep the spender XOR + zweckbindung_text superRefine.
  - In `createSpende`: after validation + member-snapshot resolution, **call `createDonation(...)`** (Phase 1) with `{ betragCents, currency, zugewendetAm, memberId, spenderName, spenderAdresse, spenderEmail, spendeKind, zweckbindungKind, zweckbindungText, wertermittlungMethode, zustandBeschreibung, herkunftsbelegFileId, belegFileId, betriebsvermoegen, projectId, actorUserId, businessId }` — and let `createDonation` derive `kategorieId`/`kategorieNameSnapshot`/`sphere='ideeller'` (do **not** pass them). Drop the `kategorien` lookup + the `projects.sphereDefault` override block entirely. Keep the `allocateBusinessId("S", year)` call. `createDonation` already fires the single append-only audit event `donation.created` (ADR-0004); **REMOVE** the now-duplicate `bus.emit("spende.created", …)` call from `createSpende` — `donation.created` is the one audit event for a created donation. Do NOT keep a parallel `spende.created` emit (append-only audit rows can't be cleaned up later → no duplicate audit rows).
  - In `editSpende`: same schema; keep the "bereits bescheinigt → 409" + festschreibung 409 guards; update the §4.3 columns directly. On a changed `spendeKind`/`zweckbindungKind` (Zweckbindung change): re-run `deriveDonationKategorieName(spendeKind, zweckbindungKind)`, resolve the derived name to a `kategorie_id` via the **same seeded lookup `createDonation` uses** (do NOT invent a parallel resolution path), and write **both** `kategorie_id` AND `kategorie_name_snapshot`. **NEVER touch `sphere_snapshot`** — Spenden are always `ideeller` (§4.5), so it is fixed at create and immutable on edit. Add one integration test: change `zweckbindungKind` on an existing Spende → assert `kategorieNameSnapshot` (and `kategorie_id`) update to the re-derived value AND `sphereSnapshot` stays `ideeller`.
...
- [ ] **Step 1: Pure (fast lane).** Most Spenden unit tests are mocked-route or DB and run in the reset lane (Step 2), so there may be **no** pure files here. Do NOT use a `tests/unit/*spenden*` glob — it would match the **C1-owned** `tests/unit/c1-spenden-status.test.ts`. Guard with explicit filenames: `files=$(git ls-files 'tests/unit/spenden-kategorie.test.ts'); [ -n "$files" ] && pnpm test:fast --run $files || echo 'no pure tests'` (replace/extend the list only with C3-owned pure files this phase actually creates; never a wildcard that could match C1/C2 suites).
