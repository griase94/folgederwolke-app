...
- **Direct-paid:** `bezahlt_von_kind='verein'` (or admin "Schon bezahlt" for member/extern) → create with `status='erstattet'`, `erstattet_am` (default today), `zahlungsart_id`, `approved_at`/`approved_by`; reuse `markExpenseAsPaid` semantics. No Erstattungsmail for Verein; optional mail for member/extern. The admin "Schon bezahlt" path stamps `approved_at = now()` and `approved_by = acting admin`, identical to the Verein-direct path, so an `erstattet` row created via either route always carries approval provenance.
...
- **Sachspende Wertermittlung** — new columns:
  ...
  - `zustand_beschreibung text` — required when `spende_kind='sachspende'`. **Holds the FULL object designation (Art + Zustand)** — i.e. _what_ the donated item is together with its condition (e.g. "Beamer Epson EB-2250U, gebraucht, voll funktionsfähig"), not the condition alone; the Muster-Zuwendungsbestätigung needs the genaue Bezeichnung. (Column name is misleading; do not rename in this pass — flag to rename later.)
  - `herkunftsbeleg_file_id uuid` → `files` — optional (separate from the main `belegFileId`).
  - `betriebsvermoegen boolean` — default `false` (= Privatvermögen). UI checkbox, **unchecked by default**. Selects the Muster-Zuwendungsbestätigung legal-text branch (Sachspende aus Betriebsvermögen vs. Privatvermögen). **Do not add a Teilwert engine** — this is a flag only.
...
- **Sachspende maps to a single Kategorie regardless of Zweckbindung** (closes the 4th-combination hole: §9.2 makes Zweckbindung mandatory for ALL Spendenarten, so the key `(spende_kind, zweckbindung_kind)` has four reachable combinations, but Sachspende collapses both `zweckfrei`/`zweckgebunden` to the one seeded "Sachspende" row — no separate `Sachspende zweckgebunden` row is seeded). The Zweckbindung text is still persisted per the §4.3 CHECK. So the derivation lookup is: `Geldspende+zweckfrei`, `Geldspende+zweckgebunden`, and `Sachspende+*` → "Sachspende".
- Seed an **"Unkategorisiert (Import)"** sentinel category (`source_kind='fixture'`) used by the importer fallback so `kategorie_id` NOT NULL never breaks `sheet_import` (see §4.6). **It is a safety net for the production importer only — the seed/test data (§4.7) never uses it; fixtures use real categories.**
...
### 4.7 Seed / fixture data — valuable, realistic, varied

All DB data is disposable test data pre-launch, so the seed should be a **showcase corpus**, not filler — every feature state demoable from a fresh `reset-test-db`. Use the **real seeded categories** (`scripts/seed.ts`, from the legacy apps-script config — this is an event/culture Verein: Eintritt, Honorar Künstler:innen, GEMA, Miete Location, Bar-Umsatz, Merch, Workshops, Sponsoring, Zuschüsse, Zinsen, …). Extend `scripts/seed-fixtures.ts` with a transaction corpus (`source_kind='fixture'`).

**Spread across 3 Buchungsjahre — 2024, 2025, 2026 — with 2024 festgeschrieben** so the year switcher, "Alle Jahre", and the read-only/Storno lock all have data to demo. Volumes realistic for ~20 people (dozens of rows/year, not thousands).

**Ausgaben — cover every status × Bezahlt-von × Beleg case:**

| Case                                            | Kategorie (real)            | Sphäre         | Bezahlt von    | Status                                     | Beleg                       |
| ----------------------------------------------- | --------------------------- | -------------- | -------------- | ------------------------------------------ | --------------------------- |
| Verein-direct, auto-paid                        | Miete Location              | zweckbetrieb   | Verein         | erstattet                                  | PDF (Rechnung)              |
| Bank fee, no receipt                            | Bankgebühren                | ideeller       | Verein         | erstattet                                  | **kein Beleg + Begründung** |
| GEMA auto-debit, no receipt                     | GEMA / Abgaben              | zweckbetrieb   | Verein         | erstattet                                  | **kein Beleg + Begründung** |
| Member reimbursement, awaiting SEPA             | Honorar Künstler:innen      | zweckbetrieb   | Mitglied       | **geprueft (offen)**                       | image                       |
| Member, aged-open (>14d, drives the pill)       | Fahrtkosten (Artists)       | zweckbetrieb   | Mitglied       | geprueft (offen)                           | image                       |
| Member, already reimbursed                      | Verpflegung (Event)         | zweckbetrieb   | Mitglied       | erstattet                                  | image                       |
| Extern payee                                    | Technik-Miete/-Kauf         | zweckbetrieb   | Extern (+IBAN) | erstattet                                  | PDF                         |
| Merch stock (WGB)                               | Merch-Einkauf / -Produktion | wirtschaftlich | Verein         | erstattet                                  | image                       |
| Pending Belegprüfung submission (not yet a row) | —                           | —              | Mitglied       | **zu_pruefen** (in `auslagen_submissions`) | image                       |
| Rejected submission                             | —                           | —              | Mitglied       | **abgelehnt**                              | image                       |

**Einnahmen — cover the sphere-split KPI (all four spheres) + the Rechnung link:**

| Case                       | Kategorie                      | Sphäre         | Link                                                                                        |
| -------------------------- | ------------------------------ | -------------- | ------------------------------------------------------------------------------------------- |
| Ticket sales (free income) | Eintritt                       | zweckbetrieb   | —                                                                                           |
| Workshop fees              | Workshop / Kursgebühr          | zweckbetrieb   | —                                                                                           |
| Bar takings                | Bar-Umsatz                     | wirtschaftlich | —                                                                                           |
| Merch sales                | Merch-Verkauf                  | wirtschaftlich | —                                                                                           |
| Public grant               | Zuschuss (zweckgebunden)       | ideeller       | linked to a Projekt                                                                         |
| Bank interest              | Zinsen                         | vermoegen      | —                                                                                           |
| Paid invoice → income      | Honorar künstlerische Leistung | zweckbetrieb   | **🔗 from `markInvoiceAsPaid`** (seed one paid invoice so the link + locked Projekt render) |

**Spenden — cover all three pickers + Bescheinigung states:**

| Case                 | Spendenart | Zweckbindung                   | Spender              | Bescheinigung                                                                         |
| -------------------- | ---------- | ------------------------------ | -------------------- | ------------------------------------------------------------------------------------- |
| Cash gift, free      | Geldspende | zweckfrei                      | Mitglied (addr auto) | **issued** (B-Nummer)                                                                 |
| Cash gift, earmarked | Geldspende | zweckgebunden (Text + Projekt) | Externe Person       | **ausstehend** (drives the pill)                                                      |
| In-kind donation     | Sachspende | zweckfrei                      | Externe Person       | ausstehend; full **Wertermittlung** (gemeiner Wert, Methode, Zustand, Herkunftsbeleg) |

**Notes:** `eur_zeile`/`anlage_gem_zeile` stay null until the Steuerberater fills them (§17) — the derived badge degrades to "Sphäre · Kategorie" without a Zeile, so don't block on it. Link some rows to seeded Projekte/Customers (already in `seed-fixtures.ts`). Ensure at least one of each spende kind, each sphere, and each Ausgaben status exists so list KPIs, filters, the "N offen"/"N ohne Bescheinigung" pills, and the bulk-paid flow are all immediately exercisable.
...
- **Bezahlt von** drives behavior — _distinct affordances to avoid the mode-error_:
  ...
  - **Mitglied / Extern** → default = Auslagenflow (`genehmigt`). An **admin-only** "Schon bezahlt?" toggle reveals a _visually distinct_ paid panel (Zahlungsart + Datum + optional "per E-Mail benachrichtigen"). Save → **erstattet**, stamping `approved_at = now()` + `approved_by = acting admin` (same as the Verein-direct path, §4.1) so the row has approval provenance. Extern shows Name + IBAN + Email instead of the member picker. (Julia is an admin → she sees this toggle; non-admin/self-service users never do.)
...
- **Unit:** filter registry → SQL predicate (per field); URL serialize/parse round-trip + Zod garbage-param handling; `(spende_kind, zweckbindung_kind) → kategorie` derivation **covering all four reachable combinations — `Geldspende+zweckfrei`, `Geldspende+zweckgebunden`, `Sachspende+zweckfrei`, and `Sachspende+zweckgebunden` (the last two both collapse to the single "Sachspende" Kategorie, §4.4)**; **`sphere = kategorie.sphere` derivation incl. a test that a project `sphere_default` does NOT change a new booking** (§4.5); beleg-or-Begründung CHECK; zweckbindung_text CHECK; "Alle Jahre"/stale-year coercion + banner suppression under Alle Jahre; duplicate-as-template payment reset; pdfjs page-render fallback path.
