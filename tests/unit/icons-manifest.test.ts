...
describe("manifest.webmanifest — icons array (PM-002, PM-015)", () => {
	...
	it("references PNG variants for 192 + 512 in both 'any' and 'maskable' purposes", () => {
		const png192Any = manifest.icons.find(
			(i) => i.src.endsWith("icon-192.png") && (i.purpose ?? "any").includes("any"),
		);
		const png512Any = manifest.icons.find(
			(i) => i.src.endsWith("icon-512.png") && (i.purpose ?? "any").includes("any"),
		);
		const png192Maskable = manifest.icons.find(
			(i) => i.src.endsWith("icon-192-maskable.png") && (i.purpose ?? "").includes("maskable"),
		);
		const png512Maskable = manifest.icons.find(
			(i) => i.src.endsWith("icon-512-maskable.png") && (i.purpose ?? "").includes("maskable"),
		);

		expect(png192Any, "manifest must reference icon-192.png as purpose 'any'").toBeDefined();
		expect(png512Any, "manifest must reference icon-512.png as purpose 'any'").toBeDefined();
		expect(
			png192Maskable,
	...
	it("each icon entry points at a file that actually exists", () => {
		for (const icon of manifest.icons) {
			...
			expect(existsSync(abs), `manifest icon ${icon.src} missing on disk`).toBe(true);
		}
	});
});
