/**
 * C2 — Server-side helper that powers the year switcher dropdown:
 *   - listAvailableYears() returns { year, closed }[] for every Buchungsjahr
 *     present in income/expenses/donations/invoices, plus the current year as
 *     a fallback when the DB has no bookings yet. `closed` reflects
...
describe.skipIf(!url)(
  "C2 listAvailableYears (JB-001/JB-003/JB-006/VB-002)",
  () => {
    ...
    afterEach(async () => {
      // Reset settings tampered with by the test rows
