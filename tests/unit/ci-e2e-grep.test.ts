...
describe("CI e2e grep coverage (F3)", () => {
  ...
  it("ci.yml e2e grep declares the core phase tags", () => {
    ...
    // Aurora implementation campaign — B3 Detail-Kette (detail-views-v4):
    expect(grepTags).toContain("@aurora-impl-b3");
    // Aurora implementation campaign — E3 Versand-Pfad (send action, mail, gate):
    expect(grepTags).toContain("@aurora-impl-e3");
    // Aurora implementation campaign — B-Kulisse (/neu dialogs over the list stage):
    ...
    // Aurora implementation campaign — C1 Mitglieder (S0–S2: matrix/detail/
    // Kassenbericht consolidation on the resolveBeitragState truth):
    expect(grepTags).toContain("@aurora-impl-c1");
  });
  ...
  const criticalSpecs = [
    ...
    // Aurora impl campaign — B3 Detail-Kette (read-by-default page, edit, delete):
    "aurora-impl-b3.spec.ts",
    // Aurora impl campaign — E3 Versand-Pfad (send round-trip + gate + Kategorie):
    "aurora-impl-e3-versand.spec.ts",
    // Aurora impl campaign — B-Kulisse (/neu dialogs over the inert list stage):
    "aurora-impl-b-kulisse.spec.ts",
    // Aurora impl campaign — B-ZB (Zuwendungsbestätigung Werkstatt round-trip):
    "aurora-impl-b-zb.spec.ts",
  ];
