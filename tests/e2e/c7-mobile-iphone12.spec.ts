...
// ---------------------------------------------------------------------------
test("@phase-7 PM-003 FAB → bottom sheet → Neue Ausgabe routes correctly", async ({
  page,
}) => {
  await signIn(page);
...
// ---------------------------------------------------------------------------
test("@phase-7 PM-003 ?kind=einnahme preselects the Einnahme type tab (C7-1)", async ({
  page,
}) => {
  await signIn(page);
...
test("@phase-7 PM-003 ?kind=spende preselects the Spende type tab (C7-1)", async ({
  page,
}) => {
  await signIn(page);
...
// ---------------------------------------------------------------------------
test("@phase-7 PM-008 at iPhone 12 width, type tabs reachable + no h-overflow", async ({
  page,
}) => {
  await signIn(page);
...
// ---------------------------------------------------------------------------
test("@phase-7 PM-009 on iPhone 12, TransactionsList renders card variant (no table)", async ({
  page,
}) => {
  await signIn(page);
  await page.goto("/app/transactions");
...
// ---------------------------------------------------------------------------
test("@phase-7 C7 MobileTabBar uses safe-area-inset-bottom padding", async ({
  page,
}) => {
  await signIn(page);
...
// ---------------------------------------------------------------------------
test("@phase-7 C7 /app renders with no horizontal overflow on iPhone 12", async ({
  page,
}) => {
  await signIn(page);
  await page.goto("/app");
