...
| `BLOB_READ_WRITE_TOKEN`                                      | Vercel Blob private-store token. Per-environment (Production points at the prod store; Preview points at a test store). | Vercel runtime                                          | Auto-provisioned by `vercel link` + `vercel blob` integration                                                    |
| `STORAGE_BACKEND`                                            | `blob` in production, `local-fs` in dev/test                                                                            | Vercel runtime + `ci.yml` build                         | `vercel env add STORAGE_BACKEND production` → `blob`                                                             |
| `GOOGLE_SERVICE_ACCOUNT_KEY_JSON`                            | Full JSON contents of `~/secrets/folgederwolke-service-account.json`. Read-only Sheets scope only.                      | Vercel runtime + `ci.yml` build                         | `cat ~/secrets/folgederwolke-service-account.json \| vercel env add GOOGLE_SERVICE_ACCOUNT_KEY_JSON production`  |
| `FINANCE_SHEET_ID`                                           | Spreadsheet ID for the `/healthz` Sheets check                                                                          | Vercel runtime + `ci.yml` build                         | `vercel env add FINANCE_SHEET_ID production` — paste spreadsheet ID                                              |
